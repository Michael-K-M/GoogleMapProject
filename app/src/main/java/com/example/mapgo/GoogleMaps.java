        ...
        switchImperial.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if(polylineDataList.size()>=SelectedRout){
                    TxtPlaceName.setText(polylineDataList.get(SelectedRout).getLeg().endAddress);
                    TxtRoute.setText("Route: " + SelectedRout);
                    TxtDuration.setText(polylineDataList.get(SelectedRout).getLeg().duration.toString());

                    if(user.getUser_isImperial().equals("false")){
                        TxtDistance.setText(polylineDataList.get(SelectedRout).getLeg().distance.toString());
                    }else {
                        double distanceInMiles= (polylineDataList.get(SelectedRout).getLeg().distance.inMeters/1609.34)*10;
